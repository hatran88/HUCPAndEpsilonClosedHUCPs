...
clock_t timeComputeUPIs;
double totalTimeComputeUPIs = 0.0f;
	...
	float utility_thres, float delta_thres, float alpha, float beta)
{
	...
	while (l >= 2 || numC > 0)
	{
		...
		for (auto oneLCand : Cl)
		{
			...
			//std::cout << "Current: " << oneLCand << " : "<< UPI << endl;
			timeComputeUPIs = clock();
			if (UPI >= utility_thres)
			{
				// Save all PCPS
				//allCPCP.insert({ oneLCand, UPI });
				// Check this pattern is or not closed
				//bool isclosed = findClosedPCPs(closedPCP, oneLCand, UPI);
				//if (isclosed)
				//{
					//closedPCP.insert({ oneLCand, UPI });
				//}
				// Check this pattern is or not delta-closed
				bool ismeta = findDeltaPats(deltaCPCP, oneLCand, UPI, delta_thres);
				if (ismeta)
				{
					deltaCPCP.insert({ oneLCand, UPI });
				}
			}
			...
			totalTimeComputeUPIs = totalTimeComputeUPIs + double(clock() - timeComputeUPIs);
		}
		// Next iterator		
		numC = candPatts.size();
